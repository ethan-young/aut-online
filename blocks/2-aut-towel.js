// AUT towel instructions
timeline.push({
  type: 'instructions',
  pages: [
    "<div style = 'text-align: left; margin: 20px 10%;'>"+
    "Thanks!<br><br>Next, please write down all of the original and creative uses for a <b style = 'color: blue;'>towel</b> that you can think of.<br><br>"+
    "Certainly, there are common, unoriginal ways to use a towel. However, for this task, list all of the unusual, creative, and uncommon uses you can think of.<br><br>" +
...
// Create the AUT towel task and loop to record RTs for each response
var aut_towel = {
  type: 'ethan-survey-text-duration',
  preamble: '<img src="img/towel.jpeg" height = "300px" ><br>Your responses:<br><br><div style="margin: 10px 10%; line-height: 40px;">',
  questions: [{prompt: 'Type your creative use here and hit enter to add it:', required: true}],
  trial_duration: 120000,
  button_label: null,
  data: {variable: "aut_towel_responses", aut: "towel", use: "", aut_stim: ""},
  on_start: function(trial){
    if(jsPsych.data.get().last(1).values()[0].aut == 'towel' & jsPsych.data.get().last(1).values()[0].use !== ''){
...
var aut_towel_rank = {
  ...
  choices: ["none"],
  data: {variable: "aut_towel_rank"},
  on_start: function(trial){
    if(trial.data.selected == "first"){
      trial.choices = jsPsych.data.get().filter({aut: 'towel'}).filterCustom(function(trial){return trial.use !== '';}).select('use').values;
      trial.data.choices = trial.choices;
    } else if(trial.data.selected == "second"){
      trial.choices = jsPsych.data.get().filter({aut: 'towel'}).filterCustom(function(trial){return trial.use !== '' & trial.use != jsPsych.data.get().last(1).values()[0].choice;}).select('use').values;
      trial.data.choices = trial.choices;
      trial.stimulus = trial.stimulus + "<b>" + jsPsych.data.get().last(1).values()[0].choice + "</b><br><br>";
    } else if(trial.data.selected == "third"){
      trial.choices = jsPsych.data.get().filter({aut: 'towel'}).filterCustom(function(trial){
...
var aut_towel_loop = {
  timeline: [aut_towel_rank, aut_towel_rerank],
  loop_function: function(data){
...
timeline.push({
  timeline: [aut_towel_loop],
  conditional_function: function(){
    var data = jsPsych.data.get().filter({aut: 'towel'}).filterCustom(function(trial){return trial.use !== '';}).select('use').values;
    if(data.length <= 3){
      return false;
    } else {
      return true;
