...
// Create the AUT paperclip task and loop to record RTs for each response
var aut_paperclip = {
...
// Rank responses 
var aut_paperclip_rank = {
    type: 'html-button-response',
    stimulus: "Which ideas are your most creative? Click on the three ideas you think are most creative.<br><br>",
    choices: ["none"],
    data: {variable: "aut_paperclip_rank"},
    on_start: function(trial){
        if(trial.data.selected == "first"){
          trial.choices = jsPsych.data.get().filter({aut: 'paperclip'}).filterCustom(function(trial){return trial.use !== '';}).select('use').values;
          trial.data.choices = trial.choices;
        } else if(trial.data.selected == "second"){
          trial.choices = jsPsych.data.get().filter({aut: 'paperclip'}).filterCustom(function(trial){return trial.use !== '' & trial.use != jsPsych.data.get().last(1).values()[0].choice;}).select('use').values;
          trial.data.choices = trial.choices;
          trial.stimulus = trial.stimulus + "<b>" + jsPsych.data.get().last(1).values()[0].choice + "</b><br><br>";
        } else if(trial.data.selected == "third"){
            trial.choices = jsPsych.data.get().filter({aut: 'paperclip'}).filterCustom(function(trial){
...
var aut_paperclip_loop = {
    timeline: [aut_paperclip_rank, aut_paperclip_rerank],
    loop_function: function(data){
...
timeline.push({
  timeline: [aut_paperclip_loop],
    conditional_function: function(){
        var data = jsPsych.data.get().filter({aut: 'paperclip'}).filterCustom(function(trial){return trial.use !== '';}).select('use').values;
        if(data.length <= 3){
            return false;
        } else {
            return true;
